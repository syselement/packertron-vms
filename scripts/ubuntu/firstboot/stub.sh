...
# Read the runner from the fetched ref, not the working tree. A fetch does not
# move the checkout, so on a retry the tree still holds whatever the first
# clone got - and a runner that was missing then stays missing forever.
#
# It goes to tmpfs rather than being run in place: the runner moves the
# checkout between revisions, and Bash re-reads a script file as it executes.
git -C "$PACKERTRON_REPO_DIR" show "origin/${PACKERTRON_REPO_BRANCH}:${RUNNER_PATH}" \
    >/run/packertron-firstboot-run ||
    {
