# Ubuntu Server 24.04 LTS template for Proxmox VE: ISO + autoinstall, sealed for cloning.
#
# Docs:
...
# Raise this first if the installer never starts: OVMF posts more slowly than SeaBIOS, and the keystrokes then land in the firmware splash, not GRUB.
variable "boot_wait" {
  type        = string
...
# - Pin it on a host with docker0/virbr0/VPN interfaces: Packer can serve the seed on an address the VM cannot reach, and the build then stalls at the installer with nothing reported wrong.
...
# Not used to log in: the seed sets `allow-pw: false`. Only piped into `sudo -S`, which the seed's NOPASSWD sudoers means sudo never reads.
...
source "proxmox-iso" "ubuntu-24-04-server" {
  ...
  # q35 rather than the i440fx default. Proxmox still exposes ide2 on q35, so the cloud-init drive below is unaffected.
  machine = "q35"
  bios    = "ovmf"
  ...
  # OVMF will not start without a variable store. pre_enrolled_keys false: with Microsoft's Secure Boot keys enrolled, anything unsigned fails to boot once cloned.
  efi_config {
  ...
  # The clone-time configuration channel: hostname, user, keys and network are set here per VM.
  cloud_init              = true
  ...
  # - The seed disables password auth and authorises one ed25519 key, and Packer cannot unlock a passphrase-protected key file - so authentication goes through the agent.
...
build {
  ...
  # The `chmod +x` prefix is Packer's own default, which overriding execute_command drops; without it every script fails with "permission denied" before running a line.
  #
  ...
  # 01 truncates the machine-id and clears cloud-init state, so a clone is treated as a fresh instance.
  provisioner "shell" {
    execute_command = "chmod +x {{ .Path }}; echo '${var.ssh_password}' | sudo -S env {{ .Vars }} {{ .Path }}"
    scripts = [
