...
- **SSH host keys** are deleted; cloud-init's `cc_ssh` regenerates them on the clone's first boot, before `sshd` starts. Shipped in the image, they let any clone impersonate any other with no warning to a client that has connected before. Verified on two clones of the same template: their ed25519 fingerprints differ. A fallback unit for a clone booted with no cloud-init drive was removed: ordered `Before=ssh.socket`, it created an ordering cycle that systemd resolved by dropping `ssh.socket`, so clones came up with no `sshd` at all - and it had never once run, because cloud-init always got there first. A clone with no cloud-init drive has no hostname, user or keys either, and is not a supported case.
